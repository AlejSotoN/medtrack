...
import { updatePatient, updatePatientAvatar, deletePatient, getPatientById, getPatients, postPatient } from './patients.service';
...
export async function patchPatientAvatar(req: Request, res: Response): Promise<void> {
    try {
        const { id } = req.params;
        ...
        const updated = await updatePatientAvatar(String(id), profilePicture);
        if (!updated) {
            res.status(404).json({ error: `Patient with ID ${id} not found.` });
            return;
        }
