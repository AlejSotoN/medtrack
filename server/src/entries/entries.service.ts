...
export async function updateEntryById(updatedEntry: Entry): Promise<Entry> {
    ...
    const values = [
      ...
      updatedEntry.entry_id, // WHERE condition
    ];
...
export async function postEntries(newEntry: NewEntry): Promise<Entry> {
    ...
    const result = await pool.query('INSERT INTO patient_entries ( patient_id, main_symptoms,condition_description, labs_asked, diagnosis, treatment, notes, date_created, last_modified) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9 ) RETURNING *', values)
